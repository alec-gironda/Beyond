...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    let explosionSound = SKAction.playSoundFileNamed("explodeSound.wav", waitForCompletion: false)
    
    let tapToStartLabel = SKLabelNode(fontNamed: "theboldfont")
    
    enum gameState{
        case preGame //game state before start of game
        case inGame //game state during game
        case afterGame //game state after game
    ...
    override func didMove(to view: SKView) {
        ...
        scoreLabel.position = CGPoint(x: self.size.width*0.22, y:self.size.height + scoreLabel.frame.height)
        ...
        livesLabel.position = CGPoint(x: self.size.width*0.78, y:self.size.height + livesLabel.frame.height)
    ...
    let amtToMovePerSec: CGFloat = 600.0
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        let amtToMoveBackground = amtToMovePerSec * CGFloat(deltaFrameTime)
        
        self.enumerateChildNodes(withName:"Background"){
            background, stop in
            
            if self.currentGameState == .inGame{
                background.position.y -= amtToMoveBackground
            }
    ...
    func spawnEnemy(){
        ...
        let loseLifeAction = SKAction.run(loseALife)
